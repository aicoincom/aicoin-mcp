...
export function registerContentTools(server: McpServer) {
  ...
  server.tool(
    ...
    'Newsflash / breaking news from AiCoin OpenData.\n• search — search newsflash by keyword. Requires: word\n• list — newsflash list with category, date, importance filters\n• detail — single newsflash full content. Requires: flash_id',
    {
      ...
      last_id: z.string().optional().describe('For list: last newsflash ID for pagination'),
      ...
      only_important: z.string().optional().describe('For list: 1=important only, 0=all (default)'),
      lan: z.string().optional().describe('For list: language (cn, en)'),
      platform_show: z.string().optional().describe('For list: platform filter (aicoin, twitter)'),
      date_mode: z.string().optional().describe('For list: normal, jump, or range'),
      jump_to_date: z.string().optional().describe('For list: YYYY-MM-DD (when date_mode=jump)'),
      start_date: z.string().optional().describe('For list: YYYY-MM-DD (when date_mode=range)'),
      end_date: z.string().optional().describe('For list: YYYY-MM-DD (when date_mode=range)'),
      ...maxItemsParam,
    },
